...
exports.testjob = function(self, message, allDone) {

	let zone = message.content.zone;
	if (!conf.zones[zone]) return allDone(true, `Unknown zone "${zone}"`);
	let job = message.content.job;
	if (!conf.zones[zone].jobs[job]) return allDone(true, `Unknown job "${nick}"`);

	let thisZone = conf.zones[zone];
	let thisJob = conf.zones[zone].jobs[job];
	let zClass = thisZone.class;

	if (zClass === "Valve") {
		let ctx = thisZone.context;
		let valve = thisJob.valve;
		let dur = thisJob.duration;
	...
	if (zClass === "Relay") {
		let ctx = thisZone.context;
		let mRelay = thisJob.relay;	
		let mState = thisJob.state;
...
exports.runall = function(self, message, allDone) {

	let zone = message.content.zone;
	if(!conf.zones[zone]) { 
		return allDone(true, `Unknown Zone "${zone}"`)
	};
	...
	if (zClass === "Relay") {
		...
		for (let job in thisZone.jobs) {
			let thisJob = thisZone.jobs[job];
			let mRelay = thisJob.relay;
			let mState = thisJob.state;

			self.nverse.ask(ctx, 'turn', {relay: mRelay, state: mState}, function (err, res) {
				msg[thisJob] = `Turned "${mRelay}" ${mState}`;
		...
		for (let job in thisZone.jobs) {
			...
			self.nverse.ask(ctx, 'queueadd', {valveName: valve, duration: dur}, function(err, res) {
				msg[thisJob] = `Added Job "${valve}" for ${dur} seconds to the Queue.`;
			});
		};
		JSON.stringify(msg);
		allDone(false, msg);
